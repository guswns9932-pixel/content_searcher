...
- 검색 키워드를 쉼표(,)나 줄바꿈으로 구분해 여러 개 입력 가능 (그중 하나라도 일치하면 결과에 표시)
- 제외 키워드: 검색 키워드가 일치해도, 파일명이나 파일 내용 어디에든(위치 무관) 제외
  키워드가 하나라도 있으면 그 파일의 결과를 전부 제외 (예: "invoice"를 찾되 파일명이나
  내용 어딘가에 "draft"가 있는 파일은 통째로 빼고 싶을 때). 쉼표로 여러 개 입력 가능
...
- 결과를 표 형태(줄무늬 스타일)로 확인하고, 여러 키워드를 입력했다면 각 행 맨 왼쪽
  "키워드" 열에서 어떤 키워드가 일치했는지 바로 확인 가능. 더블클릭으로 파일/폴더 바로 열기
- 검색 결과 CSV 저장
...
2. 검색 키워드를 입력합니다. `invoice, receipt`처럼 쉼표로 구분하면 여러 키워드 중
   하나라도 일치하는 결과를 모두 보여줍니다. 필요한 옵션(하위 폴더, 대소문자, 와일드카드, 파일명/내용)도 선택합니다.
3. 특정 단어가 같이 있는 결과는 빼고 싶다면 "제외 키워드"에 입력합니다(쉼표로 여러 개 가능).
4. 특정 형식만 검색하려면 "확장자 필터"에 `txt`, `.pdf`, `*.docx`처럼 쉼표/공백으로 구분해 입력합니다.
5. "검색 시작"을 누르면 결과가 실시간으로 표에 채워집니다.
6. 결과를 더블클릭하면 파일이 열리고, "CSV 저장"으로 결과를 내보낼 수 있습니다.
